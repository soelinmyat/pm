...
3. Decompose work into independently testable issues with dependencies, owned files, AC traces, and Test hooks. Copy `session.context.design_context` verbatim into the sidecar: approved design requirements, prototype path and SHA-256 identity (or explicit `null`), critical states, and visual invariants. Never reconstruct those values from the proposal prose or from memory. Use raw sub-issue design/spec review only when existing proposal detail is insufficient; do not spawn one worker per trivial issue.

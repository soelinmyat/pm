...
8. Run `scripts/rfc-sidecar-check.js` with sidecar, HTML, slug, and `--repo-root {artifact_repo_root}` so it recomputes any prototype binding from repository bytes. The runner also checks the sidecar context against intake. Cross-check HTML issue-card count against sidecar `issues.length`. Fix at most two bounded validation failures; otherwise record a blocker with validator output.

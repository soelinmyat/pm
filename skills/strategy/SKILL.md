...
Read `${CLAUDE_PLUGIN_ROOT}/references/writing.md` before generating any output (strategy doc, strategy deck, or strategy updates).
...
1. **Read the template** from `${CLAUDE_PLUGIN_ROOT}/references/templates/strategy-deck.html`. This is a self-contained HTML file with placeholder tokens (`{{DECK_PRODUCT_NAME}}`, `{{DECK_ICP_TITLE}}`, etc.).
...
**Slide content rules:** Follow the slide rules in `${CLAUDE_PLUGIN_ROOT}/references/writing.md` (section: "Slide content rules"). Additionally:

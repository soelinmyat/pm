...
Spawn investigation agents in batches of 10 (or fewer if less than 10 bugs). Each agent: `subagent_type: general-purpose, model: haiku`.
...
**Each approved bug gets its own sub-agent** (`subagent_type: general-purpose, model: sonnet`). Run them **sequentially** (one at a time) to prevent git conflicts.

...
This guide is followed by both the parent `pm:research` skill (inline profiling) and researcher subagents (parallel profiling). Each competitor gets its own directory at `pm/competitors/{slug}/` containing five files.
...
Never write to `pm/competitors/index.md` — that is owned by the parent skill.

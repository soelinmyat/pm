---
name: dev
description: Use when starting any development work — new features, bug fixes, refactors, or backfilling tests. Orchestrates the full lifecycle from intake through PR, review, auto-merge, and retro with inside-out TDD.
---

# Dev — Development Lifecycle

Master orchestrator for project development. Manages the full lifecycle with bundled skills for each stage.

**Hard rules:**
- **Protect the orchestrator's context window in /dev-epic.** In epic orchestration, each sub-issue's planning and implementation agent MUST be dispatched as a **team member** (`Agent` with `team_name`) — not a sub-agent. Team members communicate via SendMessage, so the orchestrator never sees their internal work. Sub-agents are fine for quick dispatches (review, code scan, spec review) that return compact results within a single /dev session. See ADR-0002.
...
- **Workspace setup** — see Stage 2: Workspace
- **Parallel dispatch** — see Stage 5: Sub-agent parallelism budget

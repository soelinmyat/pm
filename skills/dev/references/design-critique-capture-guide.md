...
Repeat `--allow-origin "https://api.example.test"` only for origins the real page needs. WebSocket access is schemeful and explicit: add its own origin, such as `--allow-origin "wss://realtime.example.test"`; allowing `https://realtime.example.test` does not implicitly allow the socket. The requested page origin is always included. Any other HTTP, HTTPS, or WebSocket origin fails the capture. The helper settles HTTP requests and WebSocket handshakes, recursively applies policy to paused child targets before they run, and terminates service workers before execution because their paused startup target cannot be isolated reliably; the main page bypasses service workers for the capture. It tracks socket frames and lifecycle changes through both screenshots and all native observations, performs a final all-target browser barrier, and keeps interception active until browser teardown. It fails on any late request, socket activity, overflow, or handler error. It also fails on redirect drift, a false or generic state assertion, a viewport mismatch, iframe content, changed source/browser/route/assertion bytes, a transparent, uniform, or near-blank screenshot, or differing decoded pixels between its two samples. Output paths are exclusive: recapture with a new capture ID instead of overwriting evidence.

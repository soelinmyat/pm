...
4. Build each execution packet with `scripts/dev-prompt.js`. For RFC-derived units, read the durable `task.work_units[].contract` and pass its acceptance criteria, approach, verification commands, test hooks, and optional `design_context` into the corresponding worker packet. For a direct approved-proposal route, use `task.design_context`; a supplied work-unit contract carries the same exact object. Never reconstruct either contract from memory. A UI worker receives the exact design requirements, source-bound prototype identity, critical states, and visual invariants. Include only current-phase material and the strict worker result schema. Do not repeat provider-specific coaching; model, effort, sandbox, and permissions come from `model-profiles.json`.

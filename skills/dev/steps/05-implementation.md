...
4. Build each execution packet with `scripts/dev-prompt.js`. For RFC-derived units, read the durable `task.work_units[].contract` and pass its acceptance criteria, approach, verification commands, and test hooks into the corresponding worker packet; never reconstruct them from memory. Include only current-phase material and the strict worker result schema. Do not repeat provider-specific coaching; model, effort, sandbox, and permissions come from `model-profiles.json`.

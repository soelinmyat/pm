...
7. **Direct approved proposal path.** For XS/S proposal work without an RFC, pass the canonical `proposal_path`. The runner derives `task.design_context` from the approved execution contract, recomputes any prototype binding, and inserts the exact context into supplied worker contracts. Do not hand-copy the context into intake facts. A caller-supplied mismatch, an approved legacy proposal without durable context, or later context/prototype drift blocks intake or resume and returns the work to Groom.

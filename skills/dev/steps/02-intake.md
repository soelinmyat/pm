...
9. Read back the route. M/L/XL proposals include readiness; tasks/bugs with adequate supplied scope may skip readiness. High-risk work uses full review regardless of kind or size. Record `ui_platform` as `web`, `mobile`, `mixed`, or `unknown`. UI impact always adds QA; explicitly assessed low-risk XS/S web UI combines visual review into QA as defined in `risk-routing.md`. All other UI retains standalone design critique. Review and verification remain mandatory.

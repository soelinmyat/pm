...
8. Write an intake facts JSON file containing `reference`, `kind`, `size`, `risk`, `acceptance_criteria`, `work_units`, and optional `proposal_path`, `non_behavioral_reason`, and `ui_platform`. For UI work, populate `ui_platform` as `web`, `mobile`, `mixed`, or `unknown` before running the route command; missing or unknown platform conservatively retains standalone design critique. Run:
...
9. Read back the route and retained `task.ui_platform`. M/L/XL proposals include readiness; tasks/bugs with adequate supplied scope may skip readiness. High-risk work uses full review regardless of kind or size. UI impact always adds QA; explicitly assessed low-risk XS/S web UI combines visual review into QA as defined in `risk-routing.md`. All other UI retains standalone design critique. Review and verification remain mandatory.

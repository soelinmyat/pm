...
2. Invoke `pm:review` with the recorded mode. `full` targets the six baseline logical lenses; `code-scan` targets bug, edge, reuse, quality, and efficiency. When the bound Dev context records `security_review_required: true`, either mode adds the distinct security lens. Physical reviewer count adapts, but applicable logical coverage and structured verdicts are mandatory. Never replace either path with an informal self-review.
...
5. Keep Review's checked `review/report.json` and, when required, `review/report.html` as the review evidence; write full-suite verification separately. Update the gate manifest with current `review` and `verification` rows, including applicable logical lenses. Structured review rows point to `review/report.json` with raw `report_sha256`; HTML rows point to `review/report.html` with the required render-manifest binding. The checker revalidates structured eligibility and all evidence and freshness requirements.
6. If earlier evidence must be recertified at the final commit, rerun the relevant check and write a phase-keyed evidence JSON file. Call `dev-session recertify --evidence <path>`; a bare commit or timestamp is never sufficient. QA recertification requires a rerun whose canonical `qa/report.json` names the final commit and passes `qa-report-check.js --qa-candidate`; a generic test record cannot stand in for QA. The candidate flag admits exactly one report run beyond the session's durable count and immutable run/commit/verdict/report-digest ledger, and successful recertification advances both. If a legacy schema-v3 session has no immutable anchors, first validate its already-accepted report at exact HEAD with `qa-report-check.js --qa-history-anchor` and call `dev-session anchor-qa-history` with that phase-keyed evidence; stale stored QA gate commits are allowed only when each identifies a passing run in the audited retained chain, and anchoring does not refresh those gate fields. Never edit or infer the ledger by hand. If a post-QA candidate fails or blocks, validate it with `qa-report-check.js --qa-candidate --allow-nonpassing` and immediately call `dev-session record-qa-nonpassing`; this advances only the history anchors, never the verified gate fields, so the fixed rerun remains an admissible one-run candidate. If relevant files changed, rerun the gate instead of recertifying it. Exception for a `review` gate that already passed: a small follow-up fix (at most 50 changed code lines inside the certified file set) may keep the frozen report through the delta-supplement protocol — `review-delta.js build`, one scoped reviewer over the delta diff, `review-delta.js record`, then a passing `review-delta.js check` (exit 0) as the recertification evidence; an amend that leaves the base unchanged needs only the passing `check`, while a rebase onto an advanced base additionally requires `--base <live authoritative base commit>` resolved from the session's persisted `source.delivery_remote` default branch (same form as `skills/ship/steps/03-review.md`) — without it the frozen base is used and the comparison cannot authenticate. `--base` is an assertion, not an input: `check` re-resolves the authoritative tip from the remote the frozen target was reviewed against and rejects any other value, so a branch-local commit cannot pose as upstream. Anything larger, or any failing check, reruns `pm:review` (see `${CLAUDE_PLUGIN_ROOT}/skills/review/references/delta-supplement.md`).

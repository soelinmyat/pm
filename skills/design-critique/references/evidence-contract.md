...
The helper keeps network interception active through the screenshot/observation window, closes it with a final settle and browser barrier, drains pending handlers, and fails on a late request, policy violation, overflow, or handler error. Accessibility, control, DOM-issue, and network collections fail rather than truncate. It also rejects transparent, uniform, and near-blank decoded images: opacity-weighted pixels are premultiplied before color, luminance, and spatial analysis; at least 1% of the effective visible coverage must differ from the dominant color bucket, and qualifying content must occupy multiple tiles with at least 1% non-dominant effective coverage in each. Distinct active states at one subject/viewport require material changed-tile coverage in addition to different hashes. For trusted native captures, a localized focus or content change can qualify without a minimum whole-viewport average distance; each changed tile must still meet the local color-distance threshold, and all native state, source, integrity, and network bindings remain required. Captures without trusted native observations and resolved product P0/P1 before/after pairs retain both material average decoded-pixel-grid distance and material changed-tile coverage.

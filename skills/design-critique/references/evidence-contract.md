...
Every schema-v2 web product-UI screenshot includes `observation: {path, sha256}` for the schema-v2 `capture.json` emitted by `scripts/design-critique-capture.js`. The closed manifest binds the exact route and capture row; the subject surface; coverage state and viewport; redacted requested, expected, and final URL identities; an exact schema-v2 declarative state assertion; native visibility/hit-test observations; CSS viewport; raw accessibility, DOM, and network files; browser executable identity before/after; clean tracked source identity before/after; plugin version; invocation configuration; ordered timestamps; screenshot byte SHA-256; canonical decoded-pixel SHA-256; and deterministic visual metrics. The URL identities retain only origin, pathname, query/fragment-presence booleans, and the full-URL SHA-256—never query or fragment values. The checker rereads every binding, recomputes the invocation, native-observation, decoded-pixel, and visual-metric identities, revalidates the current browser executable and committed Git HEAD/tree, and requires the registered normalized audits to use the raw files named by this manifest.

The state assertion's `subject_id`, `coverage_id`, and `state` exactly match the routed row. Its mandatory `state_marker` is an `id` or `test-id` whose `data-pm-state` equals that state, so a semantically generic assertion cannot certify a routed state. The marker and every explicit `visible` guard must pass ancestor visibility, effective opacity, clipping, visual-viewport intersection, and native Chromium hit-testing. Hit-testing samples five points and accepts the target or a descendant; it rejects full overlay coverage but is not a complete proof against irregular or partial occlusion between sample points.

The helper keeps network interception active through the screenshot/observation window, closes it with a final settle and browser barrier, drains pending handlers, and fails on a late request, policy violation, overflow, or handler error. Accessibility, control, DOM-issue, and network collections fail rather than truncate. It also rejects transparent, uniform, and near-blank decoded images. Distinct active states at one subject/viewport and resolved product P0/P1 before/after pairs require a material decoded-pixel-grid distance in addition to different hashes.

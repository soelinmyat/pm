...
4. Re-run deterministic checks and create a new Primary/Fresh Eyes pair. Both perspectives receive the complete current active capture set so regressions outside the fixed region remain visible. Primary may receive prior finding references for verification; Fresh Eyes still receives no previous findings, audits, acceptance context, implementation rationale, or review history. Use new context and invocation identities for both results.

...
3. For each schema-v2 web product-UI row, write a closed declarative state assertion, then run `scripts/design-critique-capture.js`. The helper captures one screenshot plus raw accessibility and DOM observations from one CDP page/session and publishes their hash-bound manifest atomically. Do not combine a manual screenshot with separately collected probes. Normalize the helper's two raw audit files with `design-critique-audit-normalize.js`; never author their checks or findings. Each normalized audit cites exactly the helper capture ID and binds the raw file named in its capture manifest. For route schema v2, every web subject needs separate `primary` desktop and `primary` narrow screenshots; a different narrow state does not substitute for the primary narrow view.
...
- Every schema-v2 web product-UI capture binds a passing trusted capture manifest whose route, state assertion, viewport, URL, source, browser, network ledger, screenshot bytes, decoded pixels, and raw observations agree.

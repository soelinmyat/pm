...
| `canary-required` | "The scheduler is installed, but its same-identity canary evidence is missing, stale, mixed, or failed." + the release-gate reason. | Default to safety: offer to pause with `/pm:loop stop`, then route to the three supervised canary commands in `install`. Never report the scheduler as safely idle. |
...
| `paused` | "The loop is paused (kill switch set)." + the STOP path if known. | Inspect `releaseGate.passed`. Only when true, offer "Resume?" → on yes, `node ${CLAUDE_PLUGIN_ROOT}/scripts/loop-install.js --project-dir "$PWD" --resume`. When false, keep the loop paused, show the release-gate reason, and route to the three supervised canary commands in `install`; never offer resume first. |

...
| `installed-idle` | The board summary + recent runs + today's budget, and "Scheduled every {interval}m · engine {engine} · merge_pr {true/false}." | "Run a cycle now, pause the loop, or open the board (`/pm:board`)?" Default: nothing (it's already scheduled) — just report. |
| `in-progress` | "In progress: {card_id} at {stage}{ on {holder}}." + the active-lease detail. | Report status; offer `/pm:board` for the live view. Don't dispatch — a wake is already holding the lease. |

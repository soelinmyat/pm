...
- **Diff identity.** A rebase or amend keeps certification when the frozen diff bytes still hash to the frozen `diff_sha256` and the current branch diff proves the same content: against the frozen base its bytes must hash to the same `diff_sha256`; against a moved live base its `git patch-id --verbatim` must equal that of the reviewed diff (`--stable` is never sufficient — it ignores intra-line whitespace). Any content change fails.
- **Delta supplements.** A post-pass fix of at most 50 changed code lines (exempt: any `test/`/`tests/`/`__tests__/` directory component at any depth, `*.test.*`/`*.spec.*` files with a JS or TS extension only, and repo-root `docs/**/*.md` only; runtime Markdown, including nested `docs/` under runtime trees, is source and stays budgeted — see `delta-supplement.md` for the exact rule) touching only the certified changed-file set may be certified by one scoped reviewer over the delta diff, recorded as `supplements/supplement-{N}.json` hash-bound to the canonical report bytes and the delta diff bytes. At most two supplements chain from one certification; the chain head must equal the current commit. See `delta-supplement.md` for the protocol and `scripts/review-delta.js` for the commands.

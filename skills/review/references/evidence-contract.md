...
- **Diff identity.** A rebase or amend keeps certification when the frozen diff bytes still hash to the frozen `diff_sha256` and the current branch diff proves the same content: against the frozen base its bytes must hash to the same `diff_sha256`; against a moved live base its `git patch-id --verbatim` must equal that of the reviewed diff (`--stable` is never sufficient — it ignores intra-line whitespace), **and** the two commits must change the same file set with a byte-identical post-image for every reviewed path. patch-id alone is not enough: it strips hunk line offsets, so the same edit relocated to another occurrence of a repeated block hashes equal against a different tree. Files the moved base itself changed may differ; a reviewed file that differs fails. Any content change fails.

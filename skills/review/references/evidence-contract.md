...
Review JSON bindings (targets, results, decisions, reports, and gate-row artifact/manifest references) use project-relative regular-file paths. Renderer payload entries inside `renders/manifest.json` deliberately retain absolute paths because they identify the exact captured files; gate validation still requires those paths to resolve to regular, non-symlink files contained by the project root. Traversal, stale bindings, unknown fields, and JSON over 4 MiB fail.

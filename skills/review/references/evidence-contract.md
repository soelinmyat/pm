...
- **Delta supplements.** A post-pass fix of at most 50 changed code lines (test files and repo-root `docs/**/*.md` exempt; runtime Markdown, including nested `docs/` under runtime trees, is source and stays budgeted) touching only the certified changed-file set may be certified by one scoped reviewer over the delta diff, recorded as `supplements/supplement-{N}.json` hash-bound to the canonical report bytes and the delta diff bytes. At most two supplements chain from one certification; the chain head must equal the current commit. See `delta-supplement.md` for the protocol and `scripts/review-delta.js` for the commands.

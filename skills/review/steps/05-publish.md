...
5. For `passed`, update only the `review` row in the canonical `.pm/dev-sessions/{slug}/gates.json`: current commit, artifact `review/report.html`, `evidence_kind: review-report-v1`, `render_manifest: review/renders/manifest.json`, its raw `render_manifest_sha256`, empty reason, checked timestamp, and applicable logical `lenses`. Preserve all other rows. Never create or update the legacy flat gate sidecar when the canonical session directory exists. For `failed` or `blocked`, record that status and concrete reason, then stop delivery.

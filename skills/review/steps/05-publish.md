...
3. Run the artifact checker and renderer with `artifact-render-check.js --marker-prefix data-review-`. Persist the renderer result at `.pm/dev-sessions/{slug}/review/renders/manifest.json`; it must bind the exact canonical `report.html`, desktop/tablet/narrow viewport and full-page PNGs, DOM metrics, browser-computed marker visibility, and print PDF. Inspect those captures and fix artifact defects before recording a pass.

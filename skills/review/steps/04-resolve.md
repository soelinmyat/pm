...
1. Check `review_round` against `iteration_cap` before any edit. At the cap, preserve the blocked report, do not mutate source, and ask the user for direction. Otherwise, auto-fix only IDs in `auto_fix_eligible`: Review-owned, confidence 80+, non-disputed, and not decision-required. Mechanical fixes are eligible; behavioral corrections also require a bound Dev acceptance contract and already-authorized implementation scope. A read-only review request alone never authorizes edits. Verify the cited code and map the correction to the agreed acceptance criteria before editing.

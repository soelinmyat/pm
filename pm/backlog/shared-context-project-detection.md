...
When the MCP server connects to the Product Memory API, it auto-detects which workspace to use by parsing the git remote URL. No manual configuration needed. The user never types a workspace ID.
...
4. If not found, prompt: "No Product Memory workspace for {slug}. Create one?" On yes, create project + S3 prefix + user_projects link.
...
**Build-on:** No existing git remote parsing in the codebase.

**Build-new:** Git remote URL parser, project lookup/creation flow, config caching. Lives in the MCP server (plugin repo) — runs locally before connecting to the API.
...
- Detection logic runs in the MCP server on the client side, then calls the API to resolve/create the workspace.

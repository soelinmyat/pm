...
1. Dashboard serves from S3 storage provider (using abstraction from PM-069).
...
7. Solo users keep localhost dashboard (zero change to current experience).

## Technical Feasibility

**Build-on:** `scripts/server.js` — entire dashboard rendering pipeline. PM-069 (storage abstraction) makes the fs→S3 swap possible. PM-071 (S3 backend) provides the storage layer.

**Build-new:** GitHub OAuth web flow (browser redirect, not device flow), project picker UI, deployment config (Dockerfile or fly.toml), production environment configuration.
...
- Depends on PM-069 (storage abstraction) and PM-071 (S3 backend).

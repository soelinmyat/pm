...
The hub API server is deployed to a cloud platform with production-grade configuration. The plugin can connect to it. DNS is configured.

## Acceptance Criteria

1. Dockerfile builds the hub server (Node.js, production deps only).
2. Deployed to Railway or Fly.io with auto-deploy on push to main.
3. Environment variables configured: S3 credentials (R2), Postgres URL (Neon), GitHub OAuth secrets, JWT signing key, Stripe keys.
...
**Risk:** Neon Postgres cold start (~500ms) on first query after idle. Acceptable for CLI but worth monitoring.

...
test("shell control keywords preserve directory changes before gated pushes", () => {
  ...
  try {
    ...
    for (const command of [
      ...
      "readonly OTHER; HOME=.; readonly -a HOME OTHER=x || unset HOME || cd gated; git push origin HEAD",
      "readonly OTHER; HOME=gated; readonly HOME OTHER=x || unset HOME || cd gated; git push origin HEAD",
      "readonly OTHER; HOME=.; export HOME OTHER=x || HOME=gated; cd; git push origin HEAD",
      "readonly OTHER; HOME=.; unset HOME OTHER || HOME=gated; cd; git push origin HEAD",
      "HOME=.; declare HOME=gated; cd; git push origin HEAD",
      "HOME=.; typeset HOME=gated; cd; git push origin HEAD",
      "CDPATH=; declare CDPATH=gated; cd child; git push origin HEAD",
    ...
    assertAllow(
      runHook("HOME=gated; declare +r HOME; unset HOME || cd gated; git push origin HEAD", {
        cwd: parent,
      })
    );
    assertBlock(

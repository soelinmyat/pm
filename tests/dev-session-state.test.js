...
test("applyRouting persists observed risk and prevents kind from erasing safeguards", () => {
  ...
  try {
    ...
    const exempt = applyRouting(session, {
      ...
      size: "XS",
      risk: { behavioral: 0 },
      non_behavioral_reason: " Documentation-only correction ",
    });
    assert.equal(exempt.task.non_behavioral_reason, "Documentation-only correction");
    assert.equal(exempt.routing.required_gates.includes("tdd"), false);
    assert.deepEqual(validateSession(exempt), []);
    exempt.task.non_behavioral_reason = " ";
    assert.notDeepEqual(validateSession(exempt), []);
  } finally {
    repo.cleanup();
  }
});

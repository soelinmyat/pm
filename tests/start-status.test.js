...
test('buildStatus prioritizes active bug-fix sessions over generic lifecycle suggestions', () => {
  ...
  try {
    project.mkdir('pm');
    project.write('.pm/config.json', '{"config_schema":1}');
    project.write('pm/research/checkout/findings.md', '---\nupdated: 2026-02-01\n---\n# Checkout\n');
    project.write('pm/backlog/idea-item.md', '---\nstatus: idea\ntitle: Idea Item\nupdated: 2026-03-15\n---\n# Idea\n');
    project.write('.pm/.groom-state.md', [
      '---',
    ...
    const bugfixPath = project.write('.pm/dev-sessions/bugfix-checkout-flow.md', [
      '| Field | Value |',
      '| --- | --- |',
      '| Stage | verify |',
      '',
    ...
    assert.deepEqual(status.alternatives, [
      '/pm:strategy',
      '/pm:refresh (1 stale items)'
    ]);
  } finally {
    project.cleanup();
  }
...
test('buildStatus detects latest groom session from .pm/groom-sessions', () => {
  ...
  try {
    project.mkdir('pm');
    project.write('.pm/config.json', '{"config_schema":1}');
    project.write('.pm/groom-sessions/older.md', [
      '---',
      'topic: Older session',
      'phase: scope',
      'updated: 2026-04-01',
    ...
    project.write('.pm/groom-sessions/newer.md', [
      '---',
      'topic: Newer session',
      'phase: team-review',
      'updated: 2026-04-03',

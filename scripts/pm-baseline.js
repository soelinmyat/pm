...
function buildBaseline(projectRoot) {
  ...
  const failedReviewSignals = steps.filter(record => {
    ...
    return step.includes('review') && (status.includes('fail') || status.includes('blocked') || status.includes('send-back'));
  }).length;
  ...
  lines.push(`- Median step duration: ${formatDuration(median(steps.map(record => Number(record.duration_ms) || 0).filter(Boolean)))}`);
  lines.push(`- Median estimated step tokens: ${median(steps.map(record => (Number(record.est_input_tokens) || 0) + (Number(record.est_output_tokens) || 0)).filter(Boolean)) || 'n/a'}`);
  ...
  if (slowestSteps.length === 0) {
    ...
    for (const record of slowestSteps) {
      lines.push(`- ${formatStepLabel(record)} — ${formatDuration(record.duration_ms)} (${record.run_id})`);
    }
  }
  lines.push('');
  ...
  const tokenHeavySteps = [...steps]
    .map(record => ({
      ...record,
      total_tokens: (Number(record.est_input_tokens) || 0) + (Number(record.est_output_tokens) || 0),
    }))
  ...
  if (tokenHeavySteps.length === 0 || tokenHeavySteps[0].total_tokens === 0) {
    ...
    for (const record of tokenHeavySteps) {
      lines.push(`- ${formatStepLabel(record)} — ${record.total_tokens} tokens (${record.token_source})`);
    }
  }
  lines.push('');
  ...
  if (retries.length === 0) {
    ...
    const groupedRetries = aggregateBy(retries, record => `${record.skill}:${record.phase || 'none'}:${record.step}`, ['duration_ms']);
  ...
  const bySkill = aggregateBy(steps, record => record.skill, ['duration_ms', 'est_input_tokens', 'est_output_tokens']);
  ...
  if (bySkill.length === 0) {
    ...
    for (const bucket of bySkill) {
      ...
      lines.push(`- ${bucket.key}: ${bucket.count} steps, ${formatDuration(bucket.duration_ms || 0)}, ${totalTokens} est tokens`);
    }
  }
  lines.push('');
...
function main() {
  ...
  if (options.output) {
    const outputPath = path.isAbsolute(options.output) ? options.output : path.join(projectRoot, options.output);

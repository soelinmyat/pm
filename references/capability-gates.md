...
| `pm:review` | built-in | Always available. In-house multi-agent review via `agent-runtime.md` (six baseline lenses: bugs, design, input edge-case, reuse, quality, efficiency — plus risk-triggered security; the last three baseline lenses absorbed the former `pm:simplify` in v1.9). Runtime-uniform — no external command dependency. No availability check needed. |

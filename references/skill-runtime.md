...
Save useful drafts without claiming user confirmation. Reuse valid decisions, evidence, and explicit user authorization across phases rather than asking again. Continue work already authorized within the same scope; ask only when a new decision or effect lacks authority. This reuse does not grant push, publication, merge, tracker, destructive, or other independent effect permissions, and does not bypass their evidence gates; changes to bound content still invalidate the relevant certification. Product approval, promotion, release authority, and exact-hash approval audits remain separate gates. A recommendation can be complete while adoption remains pending.

...
- A Think origin may retain its own slug when an approved Groom proposal keeps a different slug; its `promoted_to` names the target proposal slug. The target remains `backlog/proposals/{target-slug}.json`, and exact approval plus source lineage must authenticate the original companion. Ideate origins still share the proposal slug because Groom replaces their backlog reader.
- New promotions retain the exact original companion JSON in `promotion.origin_decision_json`, bounded to 4 MiB and verified against `origin_decision_sha256`. When the origin also supplies review evidence, proposal readers resolve locators against these exact approved bytes. Later lifecycle text never replaces approved evidence; missing or altered snapshots fail closed. Legacy companions remain readable under their original lineage rules.
